...
describe('WikiClient', () => {
    ...
    describe('fetchMultipleCardTypePages', () => {
        const pageIds = [1, 2];
        const multipleCardTypePagesParams: QueryParams = {
            action: 'query',
            format: 'json',
            pageids: '1|2',
            prop: 'info|revisions',
        ...
        it('should fetch multiple card type pages correctly', async () => {
            await wikiClient.fetchMultipleCardTypePages(pageIds);

            // eslint-disable-next-line @typescript-eslint/unbound-method
            expect(axiosSpy.get).toHaveBeenCalledWith('', { params: multipleCardTypePagesParams });
        });

        it('should return all defined card type pages', async () => {
            const queryResult: QueryResult<CardTypePage> = {
                query: {
                    pages: {
                        '1': cardTypePage,
            ...
            const actual = await wikiClient.fetchMultipleCardTypePages(pageIds);
        ...
        it(
            ...
            async () => {
                ...
                const firstQueryResult: QueryResult<CardTypePage> = {
                    query: {
                        pages: createPages(firstPageIds, cardTypePage),
                    },
                };
                const secondQueryResult: QueryResult<CardTypePage> = {
                    query: {
                        pages: createPages(secondPageIds, cardTypePage),
                    },
                };
                const firstAxiosResponse: AxiosResponse<QueryResult<CardTypePage>> = {
                    data: firstQueryResult,
                } as AxiosResponse<QueryResult<CardTypePage>>;
                const secondAxiosResponse: AxiosResponse<QueryResult<CardTypePage>> = {
                    data: secondQueryResult,
                } as AxiosResponse<QueryResult<CardTypePage>>;
                axiosSpy.get
                    .withArgs('', { params: firstParams })
                ...
                axiosSpy.get
                    .withArgs('', { params: secondParams })
                    .and.resolveTo(secondAxiosResponse);
                const expected = Array.from(pageIds, () => cardTypePage);
